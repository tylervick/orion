...
enum HistoryItemMigrationPlan: SchemaMigrationPlan {
    ...
    static let v1ToV2 = MigrationStage.custom(fromVersion: HistoryItemSchemaV1.self, toVersion: HistoryItemSchemaV2.self, willMigrate: nil) { context in
        let oldItems = try context.fetch(FetchDescriptor<HistoryItemSchemaV1.HistoryItem>())
        
        for oldItem in oldItems {
            let newItem = HistoryItemSchemaV2.HistoryItem(url: oldItem.url?.absoluteString, title: oldItem.title, visitTime: oldItem.visitTime)
